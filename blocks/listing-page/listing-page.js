    ...
    ${
      article.image !== '0' && article.image !== ''
        ? `<div class="listing-card-image-wrapper">
          <a href="${article.path}">
            <img class='listing-card-image' src="${article.image}" alt='Article Imgae'/>
          </a>
        </div>`
        : ''
    }
...
export default async function decorate(block) {
  ...
  const contenthubCategory = createCategorySlug(getMetadata('category'));
